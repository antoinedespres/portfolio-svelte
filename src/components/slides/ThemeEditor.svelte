<script lang="ts">
	...
	let brAmount = $state(3);
	let bThickness = $state(0);
	let sBlur = $state(10);
	let sOpac = $state(0.1);
	let sOffX = $state(0);
	let sOffY = $state(1);
	let font = $state("'Jost', sans-serif");
</script>
...
<Slide {gotoSlide}>
	...
	<div class="editor-body">
		<div class="row wrap">
			...
			<div class="col wrap" id="fonts">
				{#each Object.keys(fonts) as key (key)}
					<div class="row">
						<Separator size="sm" />
	...
	<div class="mobile-disclaimer col center-x">
		<h1>Oopsie 📱</h1>
		<Separator size="md" />
		<p>Sorry, the theme editor is only available on large horizontal screens for now.</p>
	</div>
</Slide>
